...
def _get_course_credit_requirements(course_key):
    ...
    try:
        sorted_block_requirements = sorted(block_requirements, key=lambda x: (x['start_date'], x['display_name']))
    except TypeError:
        # some of the blocks don't have start date set on them so sort blocks
        # on just the 'display_name' field
        sorted_block_requirements = sorted(block_requirements, key=lambda x: (x['display_name']))
            ...
            requirements.append(
                {
                    ...
                    'start_date': proctored_block.start if proctored_block else None,
                    'criteria': {},
                })
